...
function getTableJson(tableName){
   ...
   const fileData = fs.readFileSync(`./database/WCA_export_${tableName}.tsv`).toString()

   const rows = fileData.split('\n').slice(1)

   for(let rowString of rows){
...
export default async function pipelineExport(){
   ...
   const wcaExport = {
      championships: filterChampionships(getTableJson('championships')),
      people: getTableJson('Persons'),
      results: getTableJson('Results'),
      competitions: getTableJson('Competitions'),
      ranksAverage: getTableJson('RanksAverage'),
      ranksSingle: getTableJson('RanksSingle')
   }
